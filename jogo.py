...
class RegularPolygon(Body):
    ...
    def get_normal_angle(self, projectile, verbose=False):
        ...
        if not self.within_collision_distance(projectile):
            return None
        angulo_entre_vertices = math.tau / self.n_edges
        angulo = math.atan2(projectile.pos[1] - self.pos[1], projectile.pos[0] - self.pos[0]) - self.angle
        idx_v = int(angulo // angulo_entre_vertices)
        p1 = self.vertices[idx_v]
        p2 = self.vertices[(idx_v - 1) % self.n_edges]
        ...
        if verbose:
            ...
            print("\tProjetil:", projectile.name, "\t== (%d, %d)" % (projectile.pos[0], projectile.pos[1]))
            ...
            print("\tDistancias:", ["  %.2f" % math.sqrt((projectile.pos[0] - v.getX())**2 + (projectile.pos[1] - v.getY())**2) for v in self.vertices])
